...
const TagsPanel: FC<{
  ...
  return (
    ...
    >
      <p>
        <i className="text-muted">Clique sur une catégorie pour afficher/masquer les différents nodes.</i>
      </p>
      <p className="buttons">
        <button className="btn" onClick={() => setTags(mapValues(keyBy(tags, "key"), () => true))}>
          <AiOutlineCheckCircle /> Tout cocher
        </button>{" "}
        <button className="btn" onClick={() => setTags({})}>
          <AiOutlineCloseCircle /> Décocher tout
        </button>
      </p>
      <ul>
        {sortedTags.map((tag) => {
